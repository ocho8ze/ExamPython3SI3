...
def Question_5(resp):
    resp = bytes.fromhex(resp = resp[resp.index("majuscule: ")+len("majuscule: "):]).decode('utf-8')
    resp_morse_decoded = []
...
def ExamPython():
    try:
        reponse =  ["ibrahim/garni/3SI3", "15/02"]
...
#fonction principale qui lance le test tant que la question 12 n'est pas atteinte
def main():
